...
def measure_insert(msg:str, target=[1,2]):
    ...
    if isinstance(target, int):
        target = [target]
    if msg not in ['START','STREAM', 'MIGRATION', 'SWITCH', 'MIGRATION_END', 'STREAM_END']:
...
#!##############################################
#!   服务迁移开始, 要先给dn2建立link，否则下面的insert会失败
#!##############################################
    #?-----------------------------------
    # time.sleep(10)
    if config.IN_MEASUREMENT:
        wait(25, msg='stream dn1')
        measure_insert('START')
    #?-----------------------------------
